...
def evaluate_perplexity(model: GPT, data_path: str, device: str, max_samples: int = 100) -> float:
    import numpy as np
...
def evaluate_generation(model: GPT, tokenizer, prompts: List[str], device: str) -> List[str]:
    generations = []
...
def run_evaluation(config_path: str, checkpoint_path: str, output_path: str):
    ...
    if "eval_prompts" in config.get("eval", {}):
        ...
        for i, (prompt, gen) in enumerate(zip(config["eval"]["eval_prompts"], generations)):
            print(f"\nPrompt {i+1}: {prompt}")
